...
def compute_avoidance(multiranger):
    """Calcula velocidades de evitación vx_avoid, vy_avoid (tipo 'push')."""
    if not AVOID_ENABLE:
        return 0.0, 0.0

    # vx: front close -> retroceder (negativo); back close -> avanzar (positivo)
    vx_avoid = 0.0
    vx_avoid += repulsion_component(multiranger.front, AVOID_DISTANCE, AVOID_GAIN, sign=-1)
        ...
        with MotionCommander(scf) as motion_commander:
            with Multiranger(scf) as multiranger:
                ...
                while keep_flying:
                    ...
                    # 6) Evitación reactiva tipo "push"
                    vx_avoid, vy_avoid = compute_avoidance(multiranger)

                    # Si hay algo muy cerca al frente, no empujar hacia delante (solo permitir retroceder)
                    if is_close(multiranger.front, MIN_SAFE_DISTANCE_FRONT):
                        vx_base = 0.0
